...
$$\begin{eqnarray}
  ...
  L3: \gamma_{00k} &=& \delta T_k + \xi_{002}V + \varsigma_{00k}, \quad \varsigma_{00k} \thicksim N(0,\tau_{3|V}^2) \\
      \gamma_{01k} &=& \xi_{010} \\
      \gamma_{10k} &=& \xi_{100}
  \end{eqnarray}$$
...
Let average number of students per classroom be denoted as $n$, average number of classrooms per school as $J$, number of schools as $K$, and proportion of schools allocated to treatment condition as ($P$), then sampling variance of the treatment effects is (Dong & Maynard, 2013, p. 52)

$$var(\delta)=\frac{\rho_3(1-R_3^2)}{P(1-P)K} + \frac{\rho_2(1-R_2^2)}{P(1-P)JK} + \frac{(1-\rho_2-\rho_3)}{P(1-P)nJK}$$

Knowing the sampling variance of the treatment effect, we can find minimum detectable effect size (MDES), statistical power, minimum required sample size (MRSS) for any level, and constrained optimal sample allocation (COSA) solutions as in the following scenarios 
...
# Plotting Type I and Type II Error Rates
```{r, message=FALSE}
...
Raudenbush, S. W., Bryk, A. S. (2002). *Hierarchical linear modeling. Applications and Data Analysis Methods* (2nd ed.) Thousands Oaks: Sage.
