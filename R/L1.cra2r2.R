...
optimal.cra2r2 <- function(...){
  .Deprecated(new="cosa.cra2r2")
  cosa.cra2r2(...)
}

# example
# cosa.cra2r2(cn=1, cJ=10, cost=560, constrain="cost", rho2=.20)

cosa.mod1n.cra2r2 <- function(cn=0, cJ=0, cost=NULL,
                                 n=NULL, J=NULL, P=NULL,
                                 nJ0=c(10,10), P0=.50,
                                 constrain="power", optimizer="auglag_slsqp", gm=2, ncase=10,
                                 power=.80, mdes=.25, alpha=.05, two.tail=TRUE,
                                 rho2, Q=NULL, g1=0, R12=0){
  # design-specific
  parms <- as.list(environment(), all=TRUE)
  .error.handler(parms)
  fun <- "cosa.mod1n.cra2r2"
  LB <- c(2, g1+2+1)
  df <- quote(n*J - J - g1 - 2)
  SSE <- if(is.null(Q)){
...
# example
# cosa.mod1n.cra2r2(cn=1, cJ=10, cost=560, constrain="cost", rho2=.20)

cosa.mod1r.cra2r2 <- function(cn=0, cJ=0, cost=NULL,
                                 n=NULL, J=NULL, P=NULL,
                                 nJ0=c(10,10), P0=.50,
                                 constrain="power", optimizer="auglag_slsqp", gm=2, ncase=10,
                                 power=.80, mdes=.25, alpha=.05, two.tail=TRUE,
                                 rho2, omega2, Q=NULL, g1=0, R12=0, RT22=0){
  # design-specific
  parms <- as.list(environment(), all=TRUE)
  .error.handler(parms)
  fun <- "cosa.mod1r.cra2r2"
  LB <- c(1, g1+4+1)
  df <- quote(J - g1 - 4)
  SSE <- if(is.null(Q)){
...
# example
# cosa.mod1r.cra2r2(cn=1, cJ=10, cost=560, constrain="cost", rho2=.20, omega2=.20)


cosa.mod2.cra2r2 <- function(cn=0, cJ=0, cost=NULL,
                                n=NULL, J=NULL, P=NULL,
                                nJ0=c(10,10), P0=.50,
                                constrain="power", optimizer="auglag_slsqp", gm=2, ncase=10,
                                power=.80, mdes=.25, alpha=.05, two.tail=TRUE,
                                rho2, Q=NULL, g2=0, R12=0, R22=0){
  # design-specific
  parms <- as.list(environment(), all=TRUE)
  .error.handler(parms)
  fun <- "cosa.mod2.cra2r2"
  LB <- c(1, g2+4+1)
  df <- quote(J - g2 - 4)
  SSE <- if(is.null(Q)){
...
# example
# cosa.mod2.cra2r2(cn=1, cJ=10, cost=560, constrain="cost", rho2=.20)
