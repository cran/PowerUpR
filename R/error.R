.error.handler <- function(x) {
  ...
  if(any(!names.x %in% c("", "n", "J", "K", "L", "g1", "g2", "g3", "g4",
                         ...
                         "q", "p", "alpha", "power", "mdes", "es", "escp", "esa",
                         "esb", "esb1", "esB", "esab", "esab1", "esaB", "esa0", "esb0", "esb10",
                         "esB0", "powera", "powerb", "powerb1", "powerB", "rhoab",
                         "rhoab1", "rhoab2", "rhob1b2", "maxiter", "abstol",
                         "two.tailed", "mc", "nsims", "ndraws"))) {
  ...
  if(any(lapply(x[idx.es], function(x)!is.numeric(x)) == TRUE) ||
     ...
     any(x[idx.es] < 0)) {
    stop("Incorrect value for for effect size", call.=FALSE)
  }
